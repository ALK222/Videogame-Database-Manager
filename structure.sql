...
DELIMITER / / CREATE FUNCTION `p1`() RETURNS varchar(10) CHARSET utf8 RETURN @p1 / / DELIMITER;
...
DELIMITER / / CREATE PROCEDURE `update_platform`(IN `c` VARCHAR(20)) BEGIN
declare currentOwn varchar(20);
set currentOwn = (
    select count(*)
    from games
    where platform like c
  );
UPDATE platforms
SET `owned` = currentOwn
WHERE `name` LIKE c;
END / / DELIMITER;
-- Volcando estructura para disparador videogames.update_platform_trigger
SET @OLDTMP_SQL_MODE = @@SQL_MODE,
  SQL_MODE = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER / / CREATE TRIGGER `update_platform_trigger`
AFTER
INSERT ON `games` FOR EACH ROW BEGIN CALL update_platform(new.platform);
END / / DELIMITER;
SET SQL_MODE = @OLDTMP_SQL_MODE;
-- Volcando estructura para vista videogames.games_per_platform
-- Eliminando tabla temporal y crear estructura final de VIEW
DROP TABLE IF EXISTS `games_per_platform`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `games_per_platform` AS
select `games`.`name` AS `Game`,
  `games`.`platform` AS `platform`,
  `games`.`release_date` AS `release_date`,
  `games`.`edition` AS `edition`
from `games`
where `games`.`platform` like `p1`();
